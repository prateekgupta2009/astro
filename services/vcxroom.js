...
var vcxutil = require('../utils/vcxutil')
...
vcxroom.getToken = function (details, callback) {
    ...
    options.path = '/v1/rooms/' + details.room_id + '/tokens';
...
vcxroom.getroomdetail = function (detail, callback) {
    var rooms = {};

    options.path='/v1/cdr/room/'+ detail.room_id
    options.method = 'GET';
    vcxutil.connectServer(options, null, function (status,data) {
        if (status === 'success')
            callback(status, data);
