...
const international_Call_order = require('../models/internationalAudioCall');
...
router.post('/create_international_call_order', async (req, res, next) => {

    try {
        const data = {
            user_id: req.body.user_id,
            astrologer_id: req.body.astrologer_id,
            call_orderid: randomize('0000'),
            date:req.body.date
        }
                ...
                dbo.collection("astrologers").find({ "_id":ObjectId(data.astrologer_id) }, { useUnifiedTopology: true })
                    .toArray(function (err, result) {
                        if (err) {
                            ...
                            international_Call_order.create(
                                videoorderdata
                            ).then(result => {
                                //   console.log(result)
                                res.send({
...
router.post('/international_Call_orderUpdate',async (req,res,next)=>{

    try{
        international_Call_order.findOneAndUpdate({ user_id: req.body.user_id,call_orderid:req.body.call_orderid }, { $inc: { callprice:parseInt(req.body.callprice),calltiming:parseInt(req.body.calltiming)},
...
router.post('/findinternationalcallorderbyuserid',async(req,res,next)=>{
    try{
        const user_id=req.body.user_id
        international_Call_order.find({user_id:user_id}) 
        .exec()
        .then(userdata=>{
            if(userdata<1){
                res.json({
 ...
 router.post('/findinternationalCallorderbyastrologerid',async(req,res,next)=>{
     try{
